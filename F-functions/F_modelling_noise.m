...
function [DSpecMBank, SignalM, Noise, Esignal, Enoise, SNRnow] = F_modelling_noise(DSpecMBank, iModel)
    global Type_br_mod Model_singl Model_diap ModelType AxTFrames Noiseamp nK_base
...
X  = AT.*sin(2*pi*cumsum(FT)*dt);           % процесс дыхания без шума
x_max = abs(max(X));
YS   = sinc(-10:0.1:10);
...
function [X]=ModelLoadSignals2()
global tx dt fr
T   = 0:dt:tx-dt;       % массив отсчётов времени
f0 = fr;
Fase = ((-1)+(1+1) * rand(1))*0.2*pi;
